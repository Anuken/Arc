...
public class GifRecorder{
    ...
    private File compileGIF(Seq<byte[]> pixmaps, int width, int height, Fi directory){
        ...
        try{
            ...
            if(encodeMP4){
                try(SeekableByteChannel out = NIOUtils.writableFileChannel(directory.absolutePath() + "/" + time + ".mp4")){
                    AWTSequenceEncoder encoder = new AWTSequenceEncoder(out, Rational.R(recordfps, 1));

                    for(byte[] pixmap : pixmaps){
                        encoder.encodeImage(toImage(pixmap, width, height));
