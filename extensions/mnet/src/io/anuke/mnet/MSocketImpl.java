...
public class MSocketImpl implements MSocket{
    ...
    public MSocketImpl(InetAddress address, int port, MSerializer serializer) throws SocketException{
        this(address, port, 1024, 7000, 2500, 100, serializer);
    }

    public MSocketImpl(InetAddress address, int port, int bufferSize, int inactivityTimeout, int pingFrequency, int resendFrequency, MSerializer serializer) throws SocketException{
        this(new JavaUDPSocket(), address, port, bufferSize, inactivityTimeout, pingFrequency, resendFrequency, serializer);
    }

    public MSocketImpl(UDPSocket udp, InetAddress address, int port, int bufferSize, int inactivityTimeout, int pingFrequency, int resendFrequency, MSerializer serializer) throws SocketException{
     ...
     */
    MSocketImpl(UDPSocket udp, InetAddress address, int port, int bufferSize){
        this.state = SocketState.NOT_CONNECTED;
        this.udp = udp;
        this.address = address;
        this.port = port;
    ...
    public boolean close(String msg){
        ...
        sendData(PacketType.build5byte(PacketType.disconnect, 0, Utils.trimDCMessage(msg, bufferSize)));
    ...
    private void launchReceiveThread(){
        new Thread(() -> {
            byte[] receiveBuffer = MSocketImpl.this.receiveBuffer;
            DatagramPacket packet = MSocketImpl.this.receivePacket;
            UDPSocket udp = MSocketImpl.this.udp;
            ...
            while(poll != null){
                ...
                if(poll instanceof PingPacket){
                    ...
                    if(dcPacket.type == DisconnectionPacket.TIMED_OUT){
                        byte[] bytes = PacketType.build5byte(PacketType.disconnect, 0, Utils.trimDCMessage(dcPacket.message, bufferSize));
                        sendData(bytes);
                    }
